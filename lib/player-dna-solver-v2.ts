...
import {validateCommercialSolverReference,type SolverBenchmarkEvidence} from "@/lib/gto-reference-validation";
...
export type SolverReference={fingerprint:string;source:"IMPORTED_REFERENCE"|"STACKUP_VALIDATED";sourceDetail?:string;actions:Partial<Record<PlayerAction,SolverActionReference>>;sizings?:Partial<Record<string,SolverActionReference>>;exploitabilityBb?:number;verifiedAt?:string;benchmark?:SolverBenchmarkEvidence};
...
let quarantinedReferences:{reference:SolverReference;issues:string[]}[]=[];
...
function structurallyValidReference(reference:SolverReference){if(!reference?.fingerprint||!reference.actions)return false;const entries=Object.values(reference.actions).filter(Boolean) as SolverActionReference[];if(!entries.length)return false;if(entries.some(entry=>!Number.isFinite(entry.frequency)||entry.frequency<0||entry.frequency>100||!Number.isFinite(entry.evBb)))return false;const sum=entries.reduce((s,e)=>s+e.frequency,0);return sum>=99&&sum<=101}
export function setSolverReferences(next:SolverReference[]){const unique=new Map<string,SolverReference>();quarantinedReferences=[];for(const reference of next){if(!structurallyValidReference(reference)){quarantinedReferences.push({reference,issues:["REFERÊNCIA COM ESTRUTURA, FREQUÊNCIAS OU EV INVÁLIDOS."]});continue}const validation=validateCommercialSolverReference(reference);if(validation.status!=="VALIDATED"){quarantinedReferences.push({reference,issues:validation.issues});continue}unique.set(reference.fingerprint,{...reference,source:"STACKUP_VALIDATED"})}references=[...unique.values()];return references.length}
export function addSolverReferences(next:SolverReference[]){return setSolverReferences([...references,...quarantinedReferences.map(x=>x.reference),...next])}
export function solverReferenceCount(){return references.length}
export function quarantinedSolverReferenceCount(){return quarantinedReferences.length}
export function solverReferenceValidationReport(){return quarantinedReferences.map(item=>({fingerprint:item.reference.fingerprint,source:item.reference.source,sourceDetail:item.reference.sourceDetail??null,issues:[...item.issues]}))}
