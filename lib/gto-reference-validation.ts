...
function validateCore(reference:SolverReferenceValidationInput){
  const issues:string[]=[];
...
export function validateCommercialSolverReference(reference:SolverReferenceValidationInput):SolverReferenceValidationResult{
  const {issues,benchmark}=validateCore(reference);
  if(benchmark&&benchmark.usageRights!=="COMMERCIAL_AUTHORIZED")issues.push("USO COMERCIAL DA REFERÊNCIA NÃO ESTÁ EXPLICITAMENTE AUTORIZADO; DADO RESTRITO A BENCHMARK/QUARENTENA.");
...
export function validateBenchmarkOnlyEvidence(reference:SolverReferenceValidationInput):SolverReferenceValidationResult{
  const {issues,benchmark}=validateCore(reference);
  if(benchmark&&benchmark.usageRights==="UNKNOWN")issues.push("DIREITOS DE USO DA FONTE DESCONHECIDOS; EVIDÊNCIA NÃO PODE ENTRAR NO CORPUS DE BENCHMARK.");
  return{status:issues.length?"REJECTED":"VALIDATED",issues,thresholds:COMMERCIAL_GTO_THRESHOLDS};
}
