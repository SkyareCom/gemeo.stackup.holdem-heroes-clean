...
export function analyzeTechnicalDecision(state:SolverSpotState,selectedAction:PlayerAction,legalActions:PlayerAction[],selectedSizing?:string):TechnicalDecision{
  ...
  const conf=confidence(state,m,tour),idx=round(strength*100,0),range=rangeProfile(state,pressure,m),blockers=blockerProfile(state,board);
  const sections:DiagnosticSection[]=[
    {key:"ESTRUTURA",label:"CONTEXTO ESTRUTURAL",text:`${state.mode} · ${state.street} · ${m.multiway?`${m.opponents+1}-WAY":"HEADS-UP"} · STACK EFETIVO ${round(m.effective,1)} BB · ${position.label}`.replace('WAY":"HEADS-UP','WAY')},
