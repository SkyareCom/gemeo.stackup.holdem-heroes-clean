...
const MIXES:{scenario:string;mix:Mix}[]=[
  {scenario:"CASH / BTN VS BB / 3-BET POT / IP",mix:{correct:"CALL",adjustable:"RAISE",incorrect:"FOLD / ALL-IN",percentages:{FOLD:12,CALL:58,RAISE:27,"ALL-IN":3}}},
  {scenario:"CASH / BLIND WAR / SB VS BB / OOP",mix:{correct:"RAISE",adjustable:"CALL",incorrect:"FOLD / ALL-IN",percentages:{FOLD:8,CALL:37,RAISE:50,"ALL-IN":5}}},
  {scenario:"CASH / CO VS BTN / SRP / OOP",mix:{correct:"CALL",adjustable:"RAISE",incorrect:"FOLD / ALL-IN",percentages:{FOLD:5,CALL:68,RAISE:24,"ALL-IN":3}}},
  {scenario:"CASH / MULTIWAY / BB VS HJ VS BTN / OOP",mix:{correct:"CALL",adjustable:"FOLD",incorrect:"RAISE / ALL-IN",percentages:{FOLD:24,CALL:62,RAISE:11,"ALL-IN":3}}},
  {scenario:"CASH / CO VS BB / 2ND BARREL / OOP",mix:{correct:"CALL",adjustable:"FOLD",incorrect:"RAISE / ALL-IN",percentages:{FOLD:31,CALL:57,RAISE:10,"ALL-IN":2}}},
  {scenario:"CASH / BTN VS BB / THIN VALUE / IP",mix:{correct:"BET",adjustable:"CHECK",incorrect:"RAISE / ALL-IN",percentages:{CHECK:34,BET:62,RAISE:3,"ALL-IN":1}}},
  {scenario:"CASH / BTN VS BB / OVERBET / BLUFF CATCH",mix:{correct:"CALL",adjustable:"FOLD",incorrect:"RAISE / ALL-IN",percentages:{FOLD:39,CALL:56,RAISE:4,"ALL-IN":1}}},
  {scenario:"CASH / MULTIWAY / SIDE POT / CO VS BTN VS SB",mix:{correct:"RAISE",adjustable:"CALL",incorrect:"FOLD / ALL-IN",percentages:{FOLD:4,CALL:39,RAISE:52,"ALL-IN":5}}},
  {scenario:"TORNEIO / EARLY GAME / ANTE 0.1 BB / UTG VS CO VS SB / MULTIWAY",mix:{correct:"RAISE",adjustable:"CALL",incorrect:"FOLD / ALL-IN",percentages:{FOLD:5,CALL:24,RAISE:66,"ALL-IN":5}}},
  {scenario:"TORNEIO / MID GAME / ANTE 0.1 BB / BLIND WAR / SB VS BB",mix:{correct:"RAISE",adjustable:"CALL",incorrect:"FOLD / ALL-IN",percentages:{FOLD:9,CALL:33,RAISE:52,"ALL-IN":6}}},
  {scenario:"TORNEIO / BOLHA / BOLHA ICM / ANTE 0.1 BB / CO VS BTN",mix:{correct:"CALL",adjustable:"FOLD",incorrect:"RAISE / ALL-IN",percentages:{FOLD:36,CALL:59,RAISE:4,"ALL-IN":1}}},
  {scenario:"TORNEIO / ITM / ANTE 0.1 BB / BTN VS BB / COMBO DRAW",mix:{correct:"RAISE",adjustable:"CALL",incorrect:"FOLD / ALL-IN",percentages:{FOLD:2,CALL:38,RAISE:55,"ALL-IN":5}}},
  {scenario:"TORNEIO / FT / FT ICM / ANTE 0.1 BB / MULTIWAY",mix:{correct:"FOLD",adjustable:"CALL",incorrect:"RAISE / ALL-IN",percentages:{FOLD:61,CALL:34,RAISE:4,"ALL-IN":1}}},
  {scenario:"TORNEIO / FT / FT ICM / SIDE POT / BTN VS SB VS BB",mix:{correct:"RAISE",adjustable:"CALL",incorrect:"FOLD / ALL-IN",percentages:{FOLD:3,CALL:41,RAISE:51,"ALL-IN":5}}},
  {scenario:"TORNEIO / BOLHA / BOLHA ICM / CO VS BTN / BLUFF CATCH",mix:{correct:"FOLD",adjustable:"CALL",incorrect:"RAISE / ALL-IN",percentages:{FOLD:54,CALL:42,RAISE:3,"ALL-IN":1}}},
  {scenario:"TORNEIO / MID GAME / ANTE 0.1 BB / BTN VS BB / IP",mix:{correct:"CHECK",adjustable:"BET",incorrect:"RAISE / ALL-IN",percentages:{CHECK:57,BET:38,RAISE:4,"ALL-IN":1}}},
];
...
function verdict(action:string,mix:Mix){
  if(contains(mix.correct,action))return{tag:"CORRETA",comment:"AÇÃO NA FAIXA DE MAIOR EV DA MATRIZ INTERNA DESTE SPOT."};
  if(contains(mix.adjustable,action))return{tag:"AJUSTÁVEL",comment:"AÇÃO DEFENSÁVEL, MAS A MATRIZ INTERNA INDICA OUTRA LINHA COM EV SUPERIOR."};
  return{tag:"EV NEGATIVO",comment:"AÇÃO FORA DA FAIXA RECOMENDADA PELA MATRIZ INTERNA DESTE SPOT."};
}
...
export default function PlayerDnaFinalController(){
  useEffect(()=>{
    ...
    const apply=()=>{
      ...
      const buttons=[
        {label:"CHECK",base:"CHECK",size:""},{label:"CALL",base:"CALL",size:""},{label:"FOLD",base:"FOLD",size:""},
      ...
      if(!selectedAction){
        ...
        const matched=MIXES.find(item=>item.scenario===scenarioText);
        analysis.className="final-analysis-card";
        if(matched){
          const judged=verdict(selectedAction,matched.mix);
          const internal=matched.mix.percentages[selectedAction];
          analysis.innerHTML=`<div class="analysis-line"><strong>${chosen} · ${judged.tag}</strong></div><div class="analysis-line">${judged.comment}</div><div class="analysis-line solver-line"><b>MATRIZ INTERNA ${internal??0}%</b><span>GTO WIZARD —</span><span>PIO —</span><span>DEEPSOLVER —</span><span>ICMIZER —</span><span>HRC —</span></div>`;
        }else{
          analysis.innerHTML=`<div class="analysis-line"><strong>${chosen} · SEM JULGAMENTO VALIDADO</strong></div><div class="analysis-line">SPOT DINÂMICO: NÃO HERDAR RESPOSTA DE OUTRO CENÁRIO.</div><div class="analysis-line solver-line"><b>SEM BASE SOLVER</b><span>GTO WIZARD —</span><span>PIO —</span><span>DEEPSOLVER —</span><span>ICMIZER —</span><span>HRC —</span></div>`;
        }
      }
    ...
    style.textContent=`
      ...
      .final-player-actions button[aria-pressed="true"]{border-color:#B8D7C2!important;box-shadow:inset 0 0 0 1px #B8D7C2!important;color:#009929!important;-webkit-text-fill-color:#009929!important}.final-player-actions button:disabled{opacity:.28!important}
      ...
      .final-analysis-card .analysis-line{display:flex!important;align-items:center!important;justify-content:center!important;min-width:0!important;padding:1px 6px!important;border-bottom:1px solid rgba(31,95,66,.55)!important;color:#ede6db!important;-webkit-text-fill-color:#ede6db!important;font-size:8px!important;line-height:1!important;text-align:center!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}.final-analysis-card .analysis-line:last-child{border-bottom:0!important}.final-analysis-card .analysis-line strong{color:#009929!important;-webkit-text-fill-color:#009929!important;font-size:8.5px!important}.final-analysis-card .solver-line{display:grid!important;grid-template-columns:1.3fr repeat(5,minmax(0,1fr))!important;gap:3px!important}.final-analysis-card .solver-line b,.final-analysis-card .solver-line span{min-width:0!important;color:#009929!important;-webkit-text-fill-color:#009929!important;font-size:6.8px!important;text-align:center!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}
      ...
      .final-player-footer{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:8px!important;margin-top:5px!important;width:100%!important}.final-player-footer button{width:100%!important;height:44px!important;min-height:44px!important;max-height:44px!important;border:1px solid #1F5F42!important;border-radius:12px!important;background:transparent!important;color:#009929!important;-webkit-text-fill-color:#009929!important;font-size:11px!important;text-align:center!important}.final-player-footer button:disabled{opacity:.35!important}
    `;
